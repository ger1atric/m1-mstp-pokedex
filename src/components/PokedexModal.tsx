...
export const PokedexModal: React.FC<PokedexModalProps> = ({ member, canEdit, onClose, onUpdate, onEditChange }) => {
  ...
  // Auto-cycle gallery images every 7 seconds in view mode
  useEffect(() => {
    if (isEditing || !member) return;
    const totalImages = 1 + (member.galleryImages?.length || 0);
    if (totalImages <= 1) return;
    const interval = setInterval(() => {
      setGalleryIndex(i => (i + 1) % totalImages);
    }, 4000);
    return () => clearInterval(interval);
  }, [isEditing, member?.id, member?.galleryImages?.length]);
  ...
  const allImages = [member.imageUrl, ...(member.galleryImages || [])];
  const currentImagePosition = galleryIndex === 0
    ? (member.imagePosition || 'center')
    : (member.galleryImagePositions?.[galleryIndex - 1] || 'center');

  const totalEditImages = 1 + (editedMember.galleryImages?.length || 0);
  const editCurrentSrc = editGalleryIndex === 0
    ? editedMember.imageUrl
    : (editedMember.galleryImages?.[editGalleryIndex - 1] || editedMember.imageUrl);
  const editCurrentPosition = editGalleryIndex === 0
    ? editedMember.imagePosition
    : editedMember.galleryImagePositions?.[editGalleryIndex - 1];
  ...
  const handlePan = (_: any, info: any) => {
    ...
    posX.set(newX);
    posY.set(newY);
    if (editGalleryIndex === 0) {
      handleChange('imagePosition', `${newX.toFixed(2)}% ${newY.toFixed(2)}%`);
    } else {
      setEditedMember(prev => {
        if (!prev) return null;
        const positions = [...(prev.galleryImagePositions || [])];
        positions[editGalleryIndex - 1] = `${newX.toFixed(2)}% ${newY.toFixed(2)}%`;
        return { ...prev, galleryImagePositions: positions };
      });
    }
  };
  ...
  const handleEditGalleryNav = (dir: 1 | -1) => {
    const nextIdx = (editGalleryIndex + dir + totalEditImages) % totalEditImages;
    setEditGalleryIndex(nextIdx);
    setNaturalAspect(null);
    const pos = nextIdx === 0
      ? editedMember.imagePosition
      : editedMember.galleryImagePositions?.[nextIdx - 1];
    const { x, y } = parsePosition(pos);
    posX.set(x);
    posY.set(y);
  };

  const handleUpdateGalleryPosition = (index: number, pos: string) => {
    setEditedMember(prev => {
      if (!prev) return null;
      const positions = [...(prev.galleryImagePositions || [])];
      positions[index] = pos;
                    ...
                    isEditing ? (
                      <div className="flex flex-col gap-2 w-full">
                        <div className="flex items-center gap-2">
                          <input
                            type="color"
                            title="Type 1 color"
                            className="w-7 h-7 rounded cursor-pointer border border-zinc-300 p-0.5 shrink-0"
                            value={editedMember.typeColors?.type1 || getTypeColor(editedMember.type1)}
                            onChange={(e) => handleChange('typeColors', { ...editedMember.typeColors, type1: e.target.value })}
                          />
                          <input
                            className="flex-1 min-w-0 bg-white/50 border border-zinc-300 rounded px-2 py-1"
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            title="Type 2 color"
                            className="w-7 h-7 rounded cursor-pointer border border-zinc-300 p-0.5 shrink-0"
                            value={editedMember.typeColors?.type2 || getTypeColor(editedMember.type2 || '')}
                            onChange={(e) => handleChange('typeColors', { ...editedMember.typeColors, type2: e.target.value })}
                          />
                          <input
                            className="flex-1 min-w-0 bg-white/50 border border-zinc-300 rounded px-2 py-1"
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span
                          ...
                          style={{ backgroundColor: member.typeColors?.type1 || getTypeColor(member.type1) }}
                        >
                          {member.type1}
                        </span>
                        {member.type2 && (
                          <span
                            className="type-badge font-pixel text-[10px] px-3 py-1.5 leading-none text-white rounded shadow-sm"
                            style={{ backgroundColor: member.typeColors?.type2 || getTypeColor(member.type2) }}
                          >
                            {member.type2}
                          </span>
                        )}
              ...
              <div className="w-full md:w-[55%] flex flex-col">
                  ...
                  <div className={cn(
                        ...
                        {allImages.map((_, i) => (
                          <button
                            ...
                            className={cn(
                              ...
                              i === galleryIndex ? "bg-white scale-125 shadow" : "bg-white/50 hover:bg-white/80"
                            )}
                          />
                        ))}
                      </div>
                    ...
                    {isEditing && (
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 z-30">
                        <div className="bg-black/40 px-4 py-1.5 rounded-full backdrop-blur-sm text-[11px] text-white font-bold uppercase tracking-wider">
                          {editGalleryIndex === 0 ? 'Main Photo' : `Gallery ${editGalleryIndex}`} — Drag to reposition
                        </div>
                        {totalEditImages > 1 && (
                          <div className="flex items-center gap-3 bg-black/60 px-4 py-2 rounded-xl backdrop-blur-md">
                            <button
                              onClick={() => handleEditGalleryNav(-1)}
                              className="text-white text-xl font-bold hover:text-white/70 transition-colors leading-none"
                            >‹</button>
                            <span className="text-white text-[11px] font-bold tabular-nums">{editGalleryIndex + 1} / {totalEditImages}</span>
                            <button
                              onClick={() => handleEditGalleryNav(1)}
                              className="text-white text-xl font-bold hover:text-white/70 transition-colors leading-none"
                            >›</button>
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (editGalleryIndex === 0) {
                                  ...
                                  handleUpdateGalleryPosition(editGalleryIndex - 1, pos);
                                  const { x, y } = parsePosition(pos);
                                  posX.set(x);
                                  posY.set(y);
                                }
                              ...
                              className={cn(
                                ...
                                editCurrentPosition === pos ? "bg-pokedex-red text-white" : "bg-white/20 text-white hover:bg-white/40"
                              )}
                            >
                              {pos}
                            </button>
                ...
                {isEditing && (
                  <div className="mt-4 bg-white/50 p-3 rounded-xl border border-white/20">
                    ...
                    <div className="mt-2">
                      <div className="flex items-center justify-between mb-2">
                        <h5 className="text-[11px] font-black text-zinc-500 uppercase tracking-widest">
                          Gallery ({editedMember.galleryImages?.length || 0} photos)
                        </h5>
                        <label className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-zinc-200 rounded text-[11px] font-bold text-zinc-600 cursor-pointer hover:bg-zinc-50 transition-colors">
                      ...
                      {editedMember.galleryImages && editedMember.galleryImages.length > 0 && (
                        <div className="flex gap-2 flex-wrap mt-1">
                          {editedMember.galleryImages.map((img, i) => {
                            const isActive = editGalleryIndex === i + 1;
                            return (
                              <div
                                key={i}
                                onClick={() => {
                                  setEditGalleryIndex(i + 1);
                                  setNaturalAspect(null);
                                  const pos = editedMember.galleryImagePositions?.[i];
                                  const { x, y } = parsePosition(pos);
                                  posX.set(x);
                                  posY.set(y);
                                }}
                                className={cn(
                                  "relative w-14 h-14 rounded overflow-hidden border-2 cursor-pointer group transition-all",
                                  isActive ? "border-pokedex-red ring-2 ring-pokedex-red/40" : "border-zinc-200 hover:border-zinc-400"
                                )}
                              >
                                <img
                                  src={img}
                                  ...
                                  style={{ objectPosition: editedMember.galleryImagePositions?.[i] || 'center' }}
                                />
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    const newImages = (editedMember.galleryImages || []).filter((_, idx) => idx !== i);
                                    const newPositions = (editedMember.galleryImagePositions || []).filter((_, idx) => idx !== i);
                                    if (editGalleryIndex === i + 1) setEditGalleryIndex(0);
                                    else if (editGalleryIndex > i + 1) setEditGalleryIndex(prev => prev - 1);
                                    setEditedMember(prev => prev ? { ...prev, galleryImages: newImages, galleryImagePositions: newPositions } : null);
                                  }}
                                  className="absolute top-0 right-0 w-5 h-5 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-[10px] font-bold transition-opacity rounded-bl"
                                >
                                  ✕
                                </button>
                              </div>
